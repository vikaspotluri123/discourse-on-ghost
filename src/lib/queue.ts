...
export class Queue {
	...
	async add<TExecutor extends (...arguments_: any[]) => any>(
		...
		const functionName = (execute.name || '<anonymous>').replace('bound ', '');
		...
		try {
			...
			return await Promise.resolve(execute(...arguments_)); // eslint-disable-line @typescript-eslint/no-unsafe-return
		} finally {
			unsubscribe();
		}
	}
